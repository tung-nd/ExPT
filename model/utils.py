...
def get_1d_sincos_pos_embed(dim, timesteps, dtype=torch.float32, max_period=10000):
    """
...
def build_mlp(dim_in, dim_hid, dim_out, depth, layernorm=False):
    modules = [nn.Linear(dim_in, dim_hid)]
    if layernorm:
        modules.append(nn.LayerNorm(dim_hid))
